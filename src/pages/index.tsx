...
export default function Home({ metaData }: { metaData: metaDataProps }) {
  ...
  useEffect(() => {
    if (searchParam === "") {
      dispatch(
        getDataPerCategory(
          `https://casino.api.pikakasino.com/v1/pika/en/games/tiles?pageNumber=${currentPage}&pageSize=${itemsPerPage}&gameCollections=all-games`
        )
      );
    }
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href={`https://localhost.com${metaData.path}`} />
      </Head>
